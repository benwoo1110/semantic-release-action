...
// src/endpoints.ts
var listAssociatedPullRequests = "GET /repos/{owner}/{repo}/commits/{commit_sha}/pulls";
...
var inputs = {
  githubToken: import_core.default.getInput("github_token", { required: true }),
  repoOwner: import_core.default.getInput("repo_owner"),
  repoName: import_core.default.getInput("repo_name"),
  versionBumpMode: import_core.default.getInput("version_bump_mode"),
  prerelease: import_core.default.getBooleanInput("prerelease"),
  tagPrefix: import_core.default.getInput("tag_prefix")
};
var octokit = import_github.default.getOctokit(inputs.githubToken);
var owner = inputs.repoOwner === "" ? import_github.default.context.repo.owner : inputs.repoOwner;
var repo = inputs.repoName === "" ? import_github.default.context.repo.repo : inputs.repoName;
var versionBumpMode = VersionBumpMode[inputs.versionBumpMode];
main().catch((err) => {
  console.error(err);
...
async function main() {
  ...
  if (versionBumpAction === 3 /* None */) {
    import_core.default.info("No release will be created.");
    return;
  }
  import_core.default.info(`Version bump action: ${versionBumpAction}`);
}
async function getVersionBumpAction() {
  if (versionBumpMode === "norelease" /* NoRelease */) {
    ...
    const associatedPrs = await octokit.request(listAssociatedPullRequests, {
      owner,
      repo,
      commit_sha: import_github.default.context.sha
    });
    if (associatedPrs.data.length === 0) {
      import_core.default.error("No PRs associated with this commit.");
      return 3 /* None */;
    }
    ...
    if (versionBumpActions.length === 0) {
      import_core.default.error("No release labels found on the PR.");
      return 3 /* None */;
    }
    if (versionBumpActions.length > 1) {
      import_core.default.error("Multiple release labels found on the PR.");
      return 3 /* None */;
    }
    ...
    import_core.default.error(`Unknown version bump mode: ${versionBumpMode}`);
    return 3 /* None */;
  }
}
